...
//| Prototype UI + core logic for Suhab Fibonacci Trader             |
//+------------------------------------------------------------------+
...
//----------------------- UI HELPERS ----------------------------------
void CreateButton(const string name, const string text, int x, int y, int w=120, int h=24) {
...
//----------------------- MAIN LOGIC -----------------------------
int OnInit() {
   CreateButton(OBJ_BTN_DRAW_ENTRY,  "Draw Entry Grid",   10,  30);
   CreateButton(OBJ_BTN_DRAW_TAKE,   "Draw Take Grid",   140,  30);
   CreateButton(OBJ_BTN_CALC,        "Calculate",        270,  30);
   CreateButton(OBJ_BTN_PLACE,       "Place Orders",     400,  30);
   CreateButton(OBJ_BTN_NEXTCFG,     "Next Config",      530,  30);
   CreateButton(OBJ_BTN_DELETECFG,   "Delete Config",    660,  30);

   CreateEdit(OBJ_EDIT_RISK, DoubleToString(DefaultRiskLimit, 2), 10, 60, 100, 20);
   CreateLabel(OBJ_LABEL_INFO, "Ready", 120, 65);

   Print("SuhabFiboTrader_v567 UI initialized");
...
void OnDeinit(const int reason) {
   ...
   Print("SuhabFiboTrader_v567 UI deinitialized");
}

//--- пока остальные функции остаются по заглушкам (реализуем шаг за шагом)
