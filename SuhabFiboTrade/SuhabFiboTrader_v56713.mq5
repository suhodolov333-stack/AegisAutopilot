...
//| UI + XY wrapper + EntryFibo + заглушки + журнал ошибок           |
//+------------------------------------------------------------------+
...
void OnDeinit(const int reason) {
   ...
   string name=GetFirstEntryObject();
   while(name!="") {
      ObjectDelete(0,name);
      name=GetNextEntryObject(name);
   }
...
//
// 30.09.2025: Ошибка "function 'GetFirstEntryObject' must have a body" снова.
// Причина: оставлены только объявления функций (forward declarations).
// Решение: добавлены заглушки с пустым телом или возвратом значения по умолчанию.
//
// 30.09.2025: Ошибка в логике CreateEntryFiboFromPoints.
// Причина: уровни записывались как коэффициенты (1.618, 2.618...) вместо цен.
// Решение: уровни теперь рассчитываются как цены по формуле L[i] = p2 + (p1 - p2)*ratio.
// Подписи на графике остаются в формате коэффициентов (или L+коэффициент для удобства).
//
// 30.09.2025: Ошибка "undeclared identifier / wrong parameters count" для OBJPROP_LEVELVALUE1..4.
// Причина: в MQL5 нет свойств OBJPROP_LEVELVALUE1..4, OBJPROP_LEVELTEXT1..4.
// Решение: использовать индексную форму ObjectSetDouble(..., OBJPROP_LEVELVALUE, index, value)
// и ObjectSetString(..., OBJPROP_LEVELTEXT, index, text).
//==================================================================
